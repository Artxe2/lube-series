import { decorator } from "./async-lube.js"
...
alreadyTest()
	...
	.catch(reason => console.log("Assert already error:", reason === "Request already in progress"))

const debounceTest = decorator(resolve) 
	.debounce(500)
debounceTest()
	.then(() => console.log("Assert debounce error:", false))
	.catch(reason => console.log("Assert debounce error:", reason === "Request be debounced"))
await debounceTest()
...
await throttleTest()
	...
	.catch(reason => console.log("Assert throttle error:", reason === "Too many requests"))
...
await retryTest()
	...
	.catch(reason => console.log("Assert retry fail:", reason === "FAIL 3"))
await retryTest()
	.then(value => console.log("Assert retry success:", value === "OK"))
	.catch(() => console.log("Assert retry fail:", false))